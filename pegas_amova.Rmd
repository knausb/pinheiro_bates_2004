...
getSSD <- function(y, gr, Nlv, N, nsamp) {
        SSD <- numeric(Nlv + 2)
    ...
    getDF <- function(gr, Nlv, N, nsamp) {
        dfreedom <- numeric(Nlv + 2)
        dfreedom[1:Nlv] <- unlist(lapply(N, length))
        dfreedom[Nlv + 1] <- nsamp
        for (i in (Nlv + 1):2) dfreedom[i] <- dfreedom[i] - dfreedom[i - 1]
...
permutation_test <- function(nperm, sigma2, y, gr){
  ...
  dfreedom <- getDF(gr, Nlv, N, nsamp)
        ...
        for (i in 1:nperm) {
            ...
            rSSD <- getSSD(rY, gr, Nlv, N, nsamp)
            rSigma2[i, j] <- getVarComp(gr, rSSD/dfreedom, Nlv, ncoef)[j]
        }
        if (Nlv > 1) {
            j <- Nlv
            ...
            if (Nlv > 2) {
                for (j in (Nlv - 1):2) {
                  ...
                  for (i in 1:nperm) {
                    ...
                    rSSD <- getSSD(rY, rGR, Nlv, rN, nsamp)
                    rDF <- getDF(rGR, Nlv, rN, nsamp)
